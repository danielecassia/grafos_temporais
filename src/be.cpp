...
void disjkstra(const int metricID, const string compare, const Graph &g,
  ...
  while (!pq.empty()) {
    ...
    if (vertice > 0) {
      if (edgeId < 0) {
        cout << "oops! edgeId < 0: " << edgeId << " no vertice " << vertice
             << endl;
      }
      parent[vertice] = edgeId;  // Atualiza o pai do vizinho
    }
...
void printGraph(const Graph &g, int N) {
  for (int i = 0; i < N; ++i) {
    ...
    for (int j = 0; j < (int)g[i].size(); j++) {
      cout << " -> (v: " << g[i][j].vertex << ", edgeID: " << g[i][j].edgeID
           << ")";
    }
    cout << endl;
  }
...
int main() {
  ...
  if (false) {
      ...
      if (i > 0 && parent[i] >= 0) {
        for (int j = 0; j < (int)g[parent[i]].size(); j++) {
          if (g[parent[i]][j].vertex == i) {
            // cout<< "MAX entre: "<< o_maximo << " e " <<
            // e[g[parent[i]][j].edgeID].year<< "= ";
    ...
    // 2. Qual o primeiro ano em que todo o reino é alcançável a partir do
    // palácio real? (todas as vilas são alcançáveis)
    // TODO: chamar o Dijkstra (ano, MAX) e olhar o max dos anos das arestas
    // utilizadas
    ...
    for (int i = 0; i < N; i++) {
      if (i > 0 && parent[i] >= 0) {
        for (int j = 0; j < (int)g[parent[i]].size(); j++) {
          if (g[parent[i]][j].vertex == i) {
            // cout<< "aresta: "<< g[parent[i]][j].edgeID<< " custo:
            // "<<e[g[parent[i]][j].edgeID].cost <<endl;
            o_maximo = max(o_maximo, e[g[parent[i]][j].edgeID].year);
          }
        }
      }
  ...
  ll custo_arestas_usadas = 0;
  for (int i = 1; i < N; i++) {
    ll edgeId = parent[i];
    ll custo_desse = e[edgeId].cost;
    cout << "custo_desse[" << edgeId << "] = " << custo_desse << endl;
    custo_arestas_usadas += custo_desse;
  }
  cout << "Custo arestas usadas : " << custo_arestas_usadas << endl;
  ...
  //       if(g[parent[i]][j].vertex == i){
  //         cout<< "aresta: "<< g[parent[i]][j].edgeID<< " custo:
  //         "<<e[g[parent[i]][j].edgeID].cost <<endl; a_soma +=
  //         e[g[parent[i]][j].edgeID].cost;
  //       }
  //     }
  //   }
  // }
