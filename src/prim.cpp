...
void prim(const Graph &g, const EdgesList &edges, vector<ll> &weight, const int N, vector<int> &parent) {
  weight.assign(N, LINF);  // Inicializa pesos com infinito
  parent.assign(N, -1);    // Inicializa pais com -1
  vector<bool> inMST(N, false);  // Marca vértices incluídos na MST

  // Fila de prioridade para escolher a próxima aresta de menor peso
  priority_queue<CAV, vector<CAV>, greater<CAV>> pq;

  // Inicia a partir do vértice 0
  pq.push({0, -1, 0});  // {custo, vértice pai, vértice atual}
  weight[0] = 0;
  ...
  while (!pq.empty()) {
    ...
    if (inMST[vertice]) continue;  // Ignora se o vértice já está na MST
    inMST[vertice] = true;

    if (pai != -1) {
      parent[vertice] = pai;  // Atualiza o pai do vértice
    }

    // Itera sobre os vizinhos do vértice atual
    for (Connection pv : g[vertice]) {
      int vizinho = pv.vertex;
      ll peso_vertice_para_vizinho = getMetric(pv, 2, edges);

      // Se o vizinho não está na MST e o peso da conexão é menor que o peso atual
      if (!inMST[vizinho] && peso_vertice_para_vizinho < weight[vizinho]) {
        weight[vizinho] = peso_vertice_para_vizinho;
        pq.push({weight[vizinho], vertice, vizinho});
      }
    }
  }
}
...
int main() {
 ...
 vector<int> parent;
  ...
  // TODO: chamar o PRIM e somar o custo das arestas utilizadas
  parent.assign(N, -1);
// Menor custo pra conectar todas as vilas
  prim(g, e, metric, N, parent);
  ll total_cost = 0;
  for (int i = 1; i < N; i++) {
    if (parent[i] != -1) {
      for (Connection pv : g[i]) {
        if (pv.vertex == parent[i]) {
          total_cost += e[pv.edgeID].cost;
          break;
        }
      }
    }
